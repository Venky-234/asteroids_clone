...
void display_start_screen(Music pixify_bgm) {
	while (!WindowShouldClose()) {
		BeginDrawing();
		ClearBackground(BLACK);
		DrawText("ASTEROIDS !!", SCREEN_WIDTH / 2 - 100, SCREEN_HEIGHT / 2 - 40, 30, WHITE);
		DrawText("Press any key to start", SCREEN_WIDTH / 2 - 150, SCREEN_HEIGHT / 2, 20, WHITE);
		if (GetKeyPressed() != 0) {
			break;
		}
		UpdateMusicStream(pixify_bgm);
		EndDrawing();
	}
}
int main() {
	...
	Music pixify_bgm = LoadMusicStream("pixify_bgm.mp3");
	SetMusicVolume(pixify_bgm, 0.5);
	PlayMusicStream(pixify_bgm);
	
	display_start_screen(pixify_bgm);
